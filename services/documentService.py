...
def extract_need(text: str) -> Optional[str]:
    return extract_section(text, r"There should be a way for a supervisor to override this logic for exceptional packs\.”([\s\S]*?)Do you have technical requirements\?")

def extract_problem(text: str) -> Optional[str]:
    return extract_section(text, r"We will develop your solution based\non this summary\.([\s\S]*?)Solution \(how we plan to solve your need\)")

def extract_solution(text: str) -> Optional[str]:
    return extract_section(text, r"This is the solution Six S is proposing\.([\s\S]*?)Components These are the individual items that makeup the proposed solution\.")
...
def extract_sections_from_doc(x_file_ref_num_or_path: Union[int, str]):
    if isinstance(x_file_ref_num_or_path, int):
        file_path = epicor_service.download_file(x_file_ref_num_or_path)
        if not file_path:
